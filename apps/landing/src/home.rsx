[logic]
let signups = create_rw_signal(0i32);
let spots_left = create_memo(move || (200 - signups.get()).max(0));
let gradient_img = std::sync::Arc::new(crate::demo_images::make_gradient(640, 400));
let checker_img = std::sync::Arc::new(crate::demo_images::make_checker(640, 400, 32));
let radial_img = std::sync::Arc::new(crate::demo_images::make_radial_alpha(640, 400));
...
col .page
    ...
    box .band fill:surface
        col .wrap
            row gap:48 wrap align:center
                col grow:1 min-width:320 gap:20
    ...
    box .band fill:surface
        col .wrap
            row gap:48 wrap align:center
                col grow:1 min-width:300
                    img src:checker_img width:100% height:280
                col grow:1 min-width:300 gap:14
