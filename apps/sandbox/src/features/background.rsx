...
col gap:20
        ...
        card gap:12
            row gap:12 align:center
                ...
                if $busy
                    spinner size:20
            row gap:10
                button label:"Compute" fill:primary on_press(|| { $busy.set(true); $total.set(String::from("…")); spawn_task(|| slow_sum(2_000_000), on_computed($total.clone(), $busy.clone())); })
    ...
    example title:"spawn_stream — many values, in order, one frame after the other"
        card gap:12
            row gap:10
                button label:"Scan" fill:primary on_press(|| { $found.set(Vec::new()); $scanning.set(true); *$scan.borrow_mut() = Some(spawn_stream(walk_project, on_found($found.clone()), on_scan_end($scanning.clone()))); })
