...
use rsx::{
    BorderRadius, Canvas, Color, DrawCommand, LayoutError, NodeVec, Paint, PathData, PathStyle,
    Point, Rect, RectPayload, RectStyle, RenderNode, Stroke, TextPayload, TextStyle, WidgetCtx,
    use_theme,
};
    ...
    Canvas::with_intrinsic_height(ctx, 580.0, |rect| {
        ...
        for (i, (&scale, &color)) in scales.iter().zip(colors.iter()).enumerate() {
            ...
            children.push(RenderNode::Transform {
                matrix,
                children: NodeVec::collect([rect_view(
                    cx - base_w * 0.5,
                    scale_y,
                    base_w,
                    base_h,
            ...
            .enumerate()
        {
            ...
            children.push(RenderNode::Transform {
                matrix,
                children: NodeVec::collect([RenderNode::Primitive(DrawCommand::Path(Arc::new(
                    rsx::PathPayload {
                        data: Arc::new(path),
                        style: PathStyle {
