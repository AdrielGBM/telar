...
thread_local! {
    // A layout-thread-local shaper used only to size text during layout. Separate
    // from the renderer's shaper (which may live on a render thread); created lazily
    // on first use because building it loads fonts.
    ...
    static MEASURE_FONTS: RefCell<Option<FontConfig>> = const { RefCell::new(None) };
}
...
pub fn set_measure_font_config(font: FontConfig) {
    MEASURE_FONTS.with(|c| *c.borrow_mut() = Some(font));
...
// Safety net: if the runtime never set a config (e.g. a preview/test path), still
// point at the platform fonts on Android so the measurer doesn't abort.
...
mod tests {
    ...
    // Constraining a box to its own measured width must not push the text onto an
    // extra line — i.e. the measured width must cover the line's full advance.
    #[test]
