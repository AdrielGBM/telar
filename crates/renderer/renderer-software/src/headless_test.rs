...
fn headless_renders_visible_pixels() {
    ...
    assert!(
        renderer.pixmap().is_some(),
        "a headless renderer draws into a pixmap"
    );
}
...
fn headless_present_is_noop_without_surface() {
    ...
    renderer.render_frame(&cmds, Some(Color::BLACK)).unwrap();
    assert!(
        renderer.read_rgba().unwrap().iter().any(|&b| b != 0),
        "the frame must have painted something, not left the buffer black"
    );
}
