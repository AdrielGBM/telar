...
pub fn greedy(text: &str, max_width: f32, width_of: impl Fn(&str) -> f32) -> (f32, usize) {
    ...
    // A newline at the very end closes the last line rather than opening another: a document lays out no line
    // box for it, and counting one made every text that ends in a break a line taller than it is drawn.
    ...
    for hard in body.split('\n') {
        ...
        for (offset, c) in hard.char_indices() {
            ...
            // Trailing space does not count towards the fit. A space at a break hangs past the edge rather
            // than pushing the word after it onto the next line, which is what every text engine does and
            // what a document does with `pre-wrap` — measuring it broke one word early, and a paragraph came
            // out a line taller than the page it was drawn on.
...
mod tests {
    ...
    fn a_trailing_space_does_not_take_a_word_to_the_next_line() {
        // Four characters in a four-character column, and then a space. Without the hang the space pushes
        // past the edge, the walk breaks on it, and the line count comes out one too high — which is the
        // twenty pixels a paragraph disagreed by, once per wrapped line.
