...
/// Held for the duration of a render thread's frame (acquire → present); concurrent with other render
/// threads, but mutually exclusive with surface lifecycle. The `()` payload has no invariants, so a panic
/// poisoning the lock is irrelevant — recover and continue.
pub fn render_guard() -> RwLockReadGuard<'static, ()> {
    GPU_LIFECYCLE.read().unwrap_or_else(|e| e.into_inner())
}

/// Held while a window/renderer surface is created or destroyed; exclusive against every render thread. The
/// caller must not already hold a render guard on this thread (would self-deadlock).
