//! The registry of laid-out interactive rects, so a surface that carves its own input region knows where the controls are.
...
reactive_core::surface_local! {
    /// A per-surface set of interactive (press/drag) targets and their laid-out rect signals. The runner activates each surface's [`InputRegionContext`] around its build/event/frame.
