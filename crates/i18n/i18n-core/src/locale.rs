...
//!
//! Mirrors `theme-core`'s active-mode store exactly: a thread-local `RwSignal` per surface, a setter, and a reactive getter. Because every translated string compiles to a `Fn() -> String` closure that calls [`use_locale`] (through [`crate::translate`]), switching the locale re-runs only the closures that read it — the same fine-grained mechanism that re-paints only the widgets reading a theme token.

use reactive_core::{RwSignal, detached, signal};

thread_local! {
    // ManuallyDrop mirrors theme-core's signals: no TLS destructor is registered, so unmapping the dylib on dlclose stays safe. Cleanup happens via reset_runtime() dropping the whole Runtime.
