...
///
/// The handle is kept rather than opened per call: on X11 and Wayland a clipboard *owner* has to stay alive to
/// serve the bytes to whoever pastes, so a handle dropped after `set_text` takes the selection with it. Behind
/// a `Mutex` because the trait is `Send + Sync` and `arboard`'s handle is not.
...
impl DesktopClipboard {
    ...
    /// Installs this as the app's clipboard, logging and carrying on where the platform has none: a missing
    /// clipboard is a paste that does nothing, not a startup that fails.
    pub fn install() {
        match Self::new() {
