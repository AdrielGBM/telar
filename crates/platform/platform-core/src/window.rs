//! What a window is, what drives one, and the traits a backend implements to supply both.

use crate::{Event, PlatformError};

#[derive(Debug, Clone, Default, PartialEq)]
/// How a window goes fullscreen, or that it does not.
...
#[derive(Debug, Clone, Default, PartialEq)]
/// Where a window opens: centred on its monitor, or at absolute coordinates.
...
#[derive(Debug, Clone)]
/// What a window is opened with: its title, size, decorations and transparency.
...
///
/// Deliberately says nothing about how it is *drawn*. A GPU or CPU renderer needs `raw-window-handle` handles out of whatever is behind this, but that is a requirement of those renderers — stated where they are built (`telar::SurfaceWindow`) — and not of every backend that can host an app. A terminal, or a canvas someone else owns, has no such handles and should not have to invent an error to return.
pub trait Window {
    fn width(&self) -> u32;
    fn height(&self) -> u32;
    ...
    /// Begins an OS-driven interactive move (drag the window by a custom title bar). Must be called while the pointer button that started the drag is still pressed. No-op on backends without a movable top-level window (layer-shell surfaces, headless).
    fn drag_window(&self) {}
    ...
    /// Brings the window to the front and requests input focus. No-op where unsupported or where the window manager forbids programmatic activation (e.g. some Wayland compositors).
    ...
    /// Whether this window renders to an offscreen target with no on-screen surface (its raw handles are unavailable). A handler must build a windowless renderer for it — a windowed renderer would fail to create a surface. Defaults to `false` (an on-screen window).
...
// Lets a multi-surface runner hold heterogeneous handlers in one map behind a single `Box<dyn EventHandler>`, without the low-level runner naming the concrete handler type.
