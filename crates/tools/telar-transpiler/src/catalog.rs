...
mod tests {
    ...
    fn an_empty_catalog_is_not_a_missing_one() {
        ...
        assert!(!unbaked.is_empty());
        assert!(unbaked.arg_names("anything").is_err());
    }
    ...
    fn a_locale_edited_since_the_bake_is_reported_once_for_the_project() {
        ...
        assert!(CatalogContext::load(&dir, "0.1.8").staleness().is_none());
        ...
        assert!(catalog.arg_names("greeting").is_err());
