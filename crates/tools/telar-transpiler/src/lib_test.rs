...
fn a_scroll_keeps_its_position_only_when_it_is_asked_to() {
    ...
    assert!(
        plain.rust_code.contains("LayoutScrollArea::new(") && !plain.rust_code.contains("new_kept"),
...
fn source_map_points_generated_view_back_to_rsx() {
    ...
    assert!(
        !result.rust_code.contains("@RSX@"),
        "a marker survived transpilation:\n{}",
        result.rust_code
    );
...
fn generates_counter() {
    ...
    let code = out.rust_code;
    assert!(
        code.contains("pub fn counter(props: CounterProps, children: Children)"),
        "{code}"
    );
    assert!(code.contains("fn style_card() -> LayoutStyle"), "{code}");
    assert!(
        code.contains("move || format!(\"Count: {}\", { count.get() })"),
        "{code}"
    );
    assert!(code.contains("button(ButtonProps::props()"), "{code}");
    assert!(code.contains(".label(\"Increment\")"), "{code}");
    assert!(
        code.contains(
    ...
    assert!(code.contains("count.update(|n| *n += 1)"), "{code}");
    assert!(
        code.contains("Container::new(style_card(), children!["),
        "{code}"
    );
    assert!(code.contains("Ok(Box::new(__col_0))"), "{code}");
}
...
fn move_clone_emitted_for_type_annotated_signal() {
    ...
    assert!(
        code.contains("let doubled = memo(move || count_rsx_mv.get() * 2);"),
        "{code}"
    );
}
...
fn expr_spans_map_interpolation_verbatim() {
    ...
    let gen_frag = &out.rust_code[span.gen_start as usize..(span.gen_start + span.len) as usize];
    assert_eq!(gen_frag, "count");
    assert!(
        out.rust_code.is_char_boundary(span.gen_start as usize),
        "gen_start {} splits a char in:\n{}",
        span.gen_start,
        out.rust_code
    );
}
...
fn expr_spans_are_char_boundary_safe_with_multibyte() {
    ...
    assert!(
        src.is_char_boundary(rs) && src.is_char_boundary(re),
        "the rsx span {rs}..{re} splits a char in: {src}"
    );
    assert!(
        out.rust_code.is_char_boundary(gs) && out.rust_code.is_char_boundary(ge),
        "the generated span {gs}..{ge} splits a char in:\n{}",
        out.rust_code
    );
...
fn an_attribute_a_tag_does_not_take_is_named() {
    ...
    let component = transpile_source("[view]\nmy_widget anything:4\n", "demo", None, None).unwrap();
    assert!(
...
fn a_cursor_is_a_keyword_or_the_expression_that_answers_with_one() {
    let named = transpile_source("[view]\nbox cursor:col_resize\n", "demo", None, None).unwrap();
    assert!(
...
fn transition_color_on_text_wraps_text_style() {
    ...
    assert!(
        code.contains(
            ".with_color({ __transition_0.retarget(theme.get().primary); __transition_0.get() })"
        ),
...
fn hex_theme_and_keyword_colors_are_unaffected_by_signal_support() {
    ...
    assert!(
        ...
        "the hex literal did not survive:\n{code}"
    );
    assert!(code.contains("Color::TRANSPARENT"), "{code}");
    assert!(code.contains("theme.get().primary"), "{code}");
}
...
fn path_tag_emits_pathdata_builder_and_widget() {
    ...
    assert!(
        code.contains(
            "PathData::new().move_to(Point::new(0.0, 0.0)).line_to(Point::new(10.0, 0.0)).close()"
        ),
