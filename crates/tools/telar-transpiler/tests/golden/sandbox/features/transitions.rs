...
pub fn features_transitions() -> Result<Box<dyn LayoutItem>, LayoutError> {
    ...
    let __col_0 = {
        let __node_0 = doc_header(DocHeaderProps { kicker: "INTERACTION", title: "Transitions", desc: "Add transition(…) to any animatable property and a value change eases over time instead of snapping. Choose a duration + easing, or a spring." })?;
        let __node_1 = {
            ...
            let __node_2 = {
                ...
                let __row_0 = {
                    ...
                    let __node_3 = button(ButtonProps { label: Box::new(move || "Toggle".to_string()), fill: Box::new(move || use_theme::<core::theme::SandboxTheme>().primary()), on_press: Box::new({ let alt = alt.clone(); let accent = accent.clone(); move || { alt.toggle(); accent.set(if alt.get() { theme().purple } else { theme().primary }) } }), ..Default::default() })?;
            ...
            let __node_4 = code_line(CodeLineProps { code: "box fill:$accent transition(fill 250ms ease-out)" })?;
            __children.push(box_item(__node_4));
            __slots.extend_default(__children);
            example(ExampleProps { title: "Color — the fill eases to its new value" }, __slots)?
        };
        let __node_5 = {
            let mut __slots = Slots::new();
            let mut __children: Vec<Box<dyn LayoutItem>> = Vec::new();
            let __node_6 = {
                ...
                let __row_1 = {
                    ...
                    let __node_7 = button(ButtonProps { label: Box::new(move || "Toggle".to_string()), fill: Box::new(move || use_theme::<core::theme::SandboxTheme>().primary()), on_press: Box::new({ let fade = fade.clone(); move || { let v = fade.peek(); fade.set(if v > 0.5 { 0.15 } else { 1.0 }) } }), ..Default::default() })?;
            ...
            let __node_8 = code_line(CodeLineProps { code: "box opacity:$fade transition(opacity 300ms ease-in-out)" })?;
            __children.push(box_item(__node_8));
            __slots.extend_default(__children);
            example(ExampleProps { title: "Opacity — the same signal, animated" }, __slots)?
        };
        let __node_9 = {
            let mut __slots = Slots::new();
            let mut __children: Vec<Box<dyn LayoutItem>> = Vec::new();
            let __col_3 = {
                let __node_10 = prop_row(PropRowProps { name: "transition(…)", values: "prop dur easing", about: "e.g. fill 250ms ease-out — runs when the value changes." })?;
                let __node_11 = prop_row(PropRowProps { name: "properties", values: "fill·stroke·color·opacity", about: "Only these animate today." })?;
                let __node_12 = prop_row(PropRowProps { name: "spring(k, c)", values: "stiffness, damping", about: "Physics curve instead of a fixed duration." })?;
                Container::new(LayoutStyle::new().flex_column().gap(6.0), children![__node_10, __node_11, __node_12])?
            };
            __children.push(box_item(__col_3));
            __slots.extend_default(__children);
            example(ExampleProps { title: "Notes" }, __slots)?
        };
