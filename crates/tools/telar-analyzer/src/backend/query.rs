...
impl Backend {
    ...
    /// The same, with the cursor resolved by `locate` instead of by section. One entry point, because only
    /// the resolution differs — an attribute *key* has no expression span of its own, so it maps to the
    /// props builder that carries its setter rather than to itself.
...
/// The offset just inside a component call's props builder, so rust-analyzer answers an attribute key with
/// the setter list — names, types and doc comments, read from the props struct itself.
///
/// An attribute *key* copies no text through, so it has no expression span to map by. What it does have is a
/// generated line: the call this element produced, where `XProps::props()` sits at the head of a chain of
/// one setter per attribute written. Landing right after that first `.` is a method completion on the
/// builder, which is exactly the question the key position is asking.
...
mod tests {
    ...
    /// The key position lands on the builder, so what comes back is the props struct's own setters rather
    /// than a table this crate would have to keep in step with every component in the workspace.
    #[test]
    ...
    /// A built-in tag never reaches here, and a line that produced no component call has no builder to point
    /// at — saying so is what keeps the caller on its own answer instead of a wrong offset.
    #[test]
