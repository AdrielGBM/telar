//! `textDocument/codeAction`: quick fixes synthesized from the diagnostics the LSP already publishes (`telar-diagnostics`). Each matches a diagnostic message and builds a `WorkspaceEdit` that inserts the missing symbol into `[style]`.
