...
fn an_omitted_prop_takes_its_declared_default() {
    ...
    assert!(!props.muted, "a bare `default` is the type's own default");
...
fn a_set_prop_wins_over_its_default() {
    ...
    assert!(props.muted, "a set prop wins over its default");
}
...
fn a_struct_with_no_required_props_still_builds() {
    assert!(
        !BareProps::props().build().flag,
        "an unset flag falls back to its default"
    );
    assert!(
        BareProps::props().flag(true).build().flag,
        "and setting it takes"
    );
}
