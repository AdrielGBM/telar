...
//! emits a runtime `rsx::i18n::translate` call. The catalog is referenced by path (`crate::__rsx_i18n::CATALOG`)
...
use quote::quote;
use rsx_transpiler::CatalogModel;
use syn::{
    Expr, LitStr, Token,
...
fn load_catalog(manifest_dir: &Path) -> Rc<Result<Option<CatalogModel>, String>> {
    ...
    let parsed = Rc::new(rsx_transpiler::parse_catalog(manifest_dir));
...
pub(crate) fn expand(input: TInput) -> TokenStream2 {
    ...
    let catalog_path: syn::Path =
        syn::parse_str(rsx_transpiler::I18N_CATALOG_PATH).expect("catalog path is valid");
    ...
    quote! {
        {
            #(#arg_lets)*
            ::rsx::i18n::translate(&#catalog_path, #key_str, &[ #(#arg_tuples),* ])
        }
    }
}
