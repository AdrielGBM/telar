...
/// `Fn(&[String]) -> Box<dyn rsx::EmbeddedApp>` — invoked once per instance with the launch args.
///
/// ```ignore
/// rsx::plugin!(|args: &[String]| -> Box<dyn rsx::EmbeddedApp> { Box::new(MyApp::new(args)) });
/// ```
///
/// The symbols are plain (release) exports — no `RSX_HOT_RELOAD_BUILD`, no `dev` feature. They use the
