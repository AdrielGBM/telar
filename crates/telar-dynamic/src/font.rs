...
///
/// The gap this closes: [`AppConfig::fonts`](https://docs.rs/telar) are faces handed over before the first frame. A face that is downloaded, picked by the user, or shipped in a language pack had nowhere to go — and a caller could not name it in a [`TextStyle`](https://docs.rs/telar) anyway, because the family a file turns out to declare is not something the caller knows.
///
