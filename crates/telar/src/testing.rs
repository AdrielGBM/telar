...
//!
//! Every application testing a Telar UI wrote the same three things: mount a tree at a size, find the text in
//! it, and decide whether something actually drew. The last one is the one that goes wrong quietly — an
//! icon-only widget draws a `Path` and no box, so a "did this render?" check that counts boxes reports it
//! blank on a machine where it renders perfectly.

use geometry_core::Rect;
use renderer_core::DrawCommand;
use ui_core::{Component, ComponentList};

/// Mounts `root` and lays it out against a `width`×`height` window, which is what the runner's first
/// `WindowResized` does — a percent-sized tree resolves to nothing until something hands it a definite space.
...
/// `Text` shapes to nothing and is the one zero-area draw that is not a fault.
///
/// Deliberately narrower than [`paints`]: only a box the layout produced can be said to have collapsed. An
/// icon's own geometry is the artwork's business — a signal-strength glyph draws its bars as filled slivers a
/// third of a pixel wide, and there is nothing wrong with that.
...
///
/// Wider than [`painted_rect`] by exactly the two commands that carry artwork: an icon-only widget draws a
/// path and nothing else. Counting only boxes makes those widgets invisible to a test rather than measured by
/// it, which is why "did this draw anything" reports them blank on a machine where they render perfectly.
...
/// Skips `what` for want of a GPU adapter — unless `TELAR_REQUIRE_GPU` is set, which turns a missing adapter
/// into a failure.
///
/// A pixel-exact test that opens with `let Ok(gpu) = gpu::open() else { return }` passes having asserted
/// nothing on a machine with no adapter, which is every CI runner that was not set up for one. This is the
/// guard that makes that a decision rather than an accident, and it is `pub` because the tests relying on it
