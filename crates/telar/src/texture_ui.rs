...
//!
//! The mirror of [`gpu::image`](crate::gpu::image). There, an application fills a texture and Telar places it in its frame; here, Telar draws inside a picture the application is assembling. Both directions leave Telar not knowing what the rest of the picture is of, which is the whole of the arrangement: it needs a *where* and a *how big*, never a *what*.
//!
//! Three independent axes fall out of it, and they are deliberately not packaged together:
//!
//! - **Where Telar composes.** Into a window, or into somebody else's texture. A shell treating its UI as a layer it composites itself, a video editor putting its UI in the frame it encodes, a thumbnail with no window and no trip through the CPU.
//! - **At what resolution.** A [`TextureUi`] lays out against its own target, so a UI at 320×180 and the window chrome around it are two trees at two resolutions in one window. A viewport that renders at half resolution while it is being dragged is the same axis from the other end.
//! - **On which grid the text lands.** That one is a `TextStyle` axis ([`Raster`](crate::Raster)), not a property of this type, so it is available to a window tree as much as to this one.
//!
//! The tree here is a real Telar tree: layout, hit-testing, scroll, overlays, motion and i18n all work. It owns its own [`Surface`], so its layout world, overlay registry and focus are its own — that is what lets it live beside a window tree on the same thread without the two reaching into each other.
//!
...
impl TextureUi {
    ...
    /// Where the texture is shown, as the rectangle it occupies in the window — the other half of [`resize`](Self::resize), and what makes a pointer land where the user sees it.
    ///
