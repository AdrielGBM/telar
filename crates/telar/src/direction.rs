...
//!
//! Lives in the facade rather than in either crate it joins: `i18n-core` is deliberately dependency-light (only `reactive-core`) and must not learn about layout, and `layout-core` resolves direction without knowing translations exist. The facade is the one place that already has both.
...
thread_local! {
    // Replaced on re-call, since a hot reload re-runs the app's setup. An `Effect` is an id, so the slot registers no TLS destructor and `dlclose` stays safe without a `ManuallyDrop`.
...
///
/// Call once at app start, next to the app's initial [`set_locale`](crate::set_locale) call. Apps that drive direction themselves (a preview harness forcing RTL, say) can skip it and call [`set_direction`](crate::set_direction) instead.
