...
//!
//! So mounting belongs to the runtime the tree lives in: [`AppRuntime::mount`](crate::AppRuntime::mount) hands the runner a [`UiTree`] it drives through this trait, and a dylib-backed app mounts a [`HotTree`] inside itself. Same shape `telar-plugin` uses for an embedded UI, and for the same reason. The seam is what removed the workaround: with the tree mounted where its signals live, nothing needs waking by hand.
