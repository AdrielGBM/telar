...
//!
//! So mounting is the app's job: [`App::mount`](crate::app::App::mount) hands the runner a [`UiTree`] it drives through this trait, and a dylib-backed app mounts a [`HotTree`] inside itself. Same shape the plugin subsystem already uses for embedded UIs (`crate::plugin::PluginInstance`). The seam is what removed the workaround: with the tree mounted where its signals live, nothing needs waking by hand.
