...
//!
//! The three predicates below are spelled out at every use rather than named once, because a `cfg` attribute
//! takes no macro: `all(feature = "desktop", not(target_os = "android"), not(target_arch = "wasm32"))` is a
//! window, `all(feature = "web", target_arch = "wasm32")` is a page, and `feature = "tui"` is a terminal.
...
///
/// Which one that is is a *build* decision: a binary compiled with `desktop` opens a window, a `wasm32` one
/// compiled with `web` mounts on a page, one compiled with only `tui` runs in the terminal it was launched
/// from. Both a window and a terminal being compiled in is the case a dev session wants — switching frontend
/// without a rebuild — and there `TELAR_TARGET=tui` picks the terminal, with the window remaining the default.
///
...
fn tui_selected() -> bool {
    ...
    let has_page = cfg!(all(feature = "web", target_arch = "wasm32"));
...
fn run_default_frontend<A: App>(_config: AppConfig, _app: A, _app_name: &str) {
    // Reachable only from a build with no frontend at all: `run_app_with_name` on a `tui`-only build never
    // gets here, because `tui_selected` answers `true` when nothing else is compiled in.
    panic!(
