...
/// How a terminal app is set up. Everything here is a property of the *terminal*, not of the application,
/// which is why none of it lives in [`AppConfig`].
#[derive(Clone, Debug)]
pub struct TuiOptions {
    /// How many logical pixels one character cell stands for. Raising it makes a layout authored for the
    /// desktop occupy fewer cells; the default is the proportion of a typical monospace face.
    pub cell: CellSize,
...
///
/// The one thing that has to happen before anything else is the measurer: layout asks how wide a string is
/// while the tree is being built, and the answer here is a count of cells rather than a shaped advance. It
/// is installed unconditionally, so it wins over the raster measurer any later font load would offer.
pub fn run_tui_app_with_name<A: App>(
    ...
    if let Err(e) = super::run_with_platform_and_renderer::<_, _, A, ()>(
        ...
        // The terminal is restored by the platform's own teardown and by its panic hook, so by the time this
        // prints, the message lands on a shell the user can read it in.
        eprintln!("telar: {e}");
        std::process::exit(1);
    }
}
