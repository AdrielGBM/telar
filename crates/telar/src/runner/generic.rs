...
///
/// The whole event → reactive → layout → render → present bridge (`AppHandler`) is already generic over the window type, so this simply loads prefs, resolves the renderer backend, applies the app's optional [`App::window_config`] override, and hands a fresh handler to `platform.run`.
///
/// The window has to be a [`SurfaceWindow`]: this entry point draws with the renderers Telar ships, and both need a real surface behind it. A backend whose window is not one uses [`run_with_platform_and_renderer`].
...
///
/// The seam for a frontend Telar knows nothing about: the factory is handed the platform's own window and returns any [`renderer_core::RenderBackend`], which the frame pipeline drives on its own thread exactly as it drives the built-in pair. Nothing here asks the window for a GPU surface, so a terminal backend can implement plain [`platform_core::Window`] and stop there.
///
/// Two consequences: `RendererBackend` and the `telar.backend` preference are ignored, and `AppCtx`'s raw window/display handles come back `None`, since this path never required the window to have any.
