...
mod tests {
    ...
    // Regression: taffy 0.11 measured a `max-width` box at its uncapped one-line intrinsic width, so this crate pinned every capped box to its resolved width and laid out a second time. That pass went with taffy 0.13; `telar/tests/max_width_wrapping.rs` covers the same case through the real stack.
    #[test]
